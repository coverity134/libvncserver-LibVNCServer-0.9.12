  ...
  if (n <= RFB_BUF_SIZE) {
    ...
    while (client->buffered < n) {
      ...
      if (client->saslconn)
        ...
        i = read(client->sock, client->buf + client->buffered, RFB_BUF_SIZE - client->buffered);//SLIVER_SOURCE
#ifdef WIN32
...
#endif
        i = read(client->sock, out, n); //SLIVER_SOURCE
...
ConnectClientToUnixSock(const char *sockFile)
{
  ...
  SLIVER_SINK(sockFile);
  strcpy(addr.sun_path, sockFile); //SLIVER_SINK
